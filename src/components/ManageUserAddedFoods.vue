...
const filteredFoods = computed(() => {
  ...
  return props.foods.filter(food => {
    const matchesName = matchesSearchQuery(food.name, searchTerm.value);
    const matchesId = String(food.id).toLowerCase().includes(searchTerm.value.toLowerCase());
...
function removeFood(index) {
  ...
  const foodToDelete = props.foods[globalIndex];
  ...
  emits('update:foods', updatedFoods, {
    description: `Deleted food item: ${foodToDelete.name}`,
    type: 'food-item',
    payload: { food: foodToDelete, trackedFoods: props.trackedFoods.filter(item => item.foodId === foodId) }
  });
