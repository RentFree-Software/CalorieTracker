...
const filteredFoods = computed(() => {
  ...
  return props.foods.filter(food =>
    matchesSearchQuery(food.name, foodSearch.value)
  );
});
...
function removeFoodItem(index) {
  ...
  const globalIndex = mealFoodsIndices[index];
  const removedItem = props.trackedFoods[globalIndex];
  const updatedFoods = props.trackedFoods.filter((_, idx) => idx !== globalIndex);
  
  emits('update:trackedFoods', updatedFoods, {
    description: `Removed tracked food: ${removedItem.foodName}`,
    type: 'tracked-food-item',
    payload: { item: removedItem }
  });
}
...
function addNewFood() {
  ...
  const fibre = parseLocaleFloat(newFoodFibre.value);
  const salt = parseLocaleFloat(newFoodSalt.value);

  if (Number.isNaN(calories) || Number.isNaN(fat) || Number.isNaN(carbohydrates) || Number.isNaN(protein) || Number.isNaN(fibre) || Number.isNaN(salt)) {
    return;
  }
      ...
      <div v-if="showAddNewDialog" class="add-dialog-overlay" role="dialog" aria-modal="true" aria-labelledby="new-food-title">
        <div class="add-dialog-box">
          ...
          <form @submit.prevent="addNewFood">
            ...
            <div class="form-group">
              <label for="new-food-fibre">Fiber (per 100 units):</label>
              <input id="new-food-fibre" v-model="newFoodFibre" type="text" inputmode="decimal" class="amount-input" required />
            </div>
